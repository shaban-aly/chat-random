...
export function RoomScreen({
  ...
  return (
    <div className="flex h-full w-full flex-col bg-background relative overflow-hidden screen-enter">
      ...
      <div 
        ref={scrollRef}
        className="flex-1 overflow-y-auto p-4 sm:p-6 pb-20 flex flex-col gap-6"
      >
        {notice && (
        ...
        {messages.map((msg) => {
          ...
          return (
            <div key={msg.id} className={`flex flex-col w-full ${isMine ? "items-end" : "items-start"}`}>
              ...
              {/* Note: In a real app we might reuse MessageBubble but Room messages need the sender's name */}
              <div
